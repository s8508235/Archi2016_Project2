...
int main()
{
    ...
    //   int wb_data = MEM2WB.instruction_op;
       ...
       // printf("%d %d\n",pc,currpc);
        ...
        if(ID2EX.isStall ==1) fprintf(out," to_be_stalled");
        if(ID2EX.forward_mess ==1) fprintf(out," fwd_EX-DM_rs_$%d",ID2EX.rs);
        else if(ID2EX.forward_mess ==2)
        {
            fprintf(out," fwd_EX-DM_rt_$%d",ID2EX.rt);
        }
        else if(ID2EX.forward_mess == 3)
        {
            fprintf(out," fwd_EX-DM_rs_$%d fwd_EX-DM_rt_$%d",ID2EX.rs,ID2EX.rt);
        }
        fprintf(out,"\n");
